...
豆知識は **320件**。目標を達成した日に1個ずつ解禁される（1日1個）。

- `KNOWLEDGE_FACTS` … 本体。形式は必ず **「本文。（※補足）」**。
  本文はやや断定気味に言い切り、数値・条件・根拠は全部 `（※…）` に逃がす。
  語尾は「〜んだ／〜だよ／〜なんだ」。断定を避けて「〜と言われています」で締める。
  絵文字は 👉 のみ、クイズの答えの直前だけ。
- `KNOWLEDGE_SERIES` … 20本のテーマ別シリーズ。**320件すべてがどれかに所属する。**
  `SERIES_ORDER` の順に1本ずつ完走していくので、**実用的なテーマを前に、雑学を後ろに**並べてある。
- `KNOWLEDGE_SERIES_NAME` … 一覧に出す表示名。シリーズを足したら必ずここにも足す。
- 一覧は `renderStamps()` が `details/summary` でシリーズごとに折りたたむ。
  進行中のシリーズだけ最初から開く。

**豆知識を足すとき**: `KNOWLEDGE_FACTS` に足すだけでは出てこない
（正確にはシリーズを全部集め終わった後に回る）。**必ず `KNOWLEDGE_SERIES` にも入れること。**
スタンプ数の分母はHTMLに直書きなので、そこも更新する。

系統別の内訳と、書ける余地の見立ては会話ログの通り。320件はほぼ上限で、
これ以上は言い換えになりやすい。
